program run_all_sims
   ...
   use mod_control_mod, only : mod_controller
...
!integer, parameter :: mod_ctrl%npts=10          !number of soil layers
...
type(pan_opts)  :: p_opts
type(h_soil_state)   :: p_state
type(ch_param)     :: brookcorey


type(mod_controller)  :: mod_ctrl
type(mod_opts)  :: my_opts
type(mass_soil_state)   :: my_state
type(bc_param)     :: clapphorn
type(tridiag_matrix) :: my_trid

real(kind=uk), allocatable, dimension(:) :: dwat,layer_dz
character(len=1024) :: def_nml = 'run.nml'

call my_opts%read_namelist(def_nml)

call brookcorey%alloc(my_opts%nlevel)
...
call mod_ctrl%alloc(my_opts,brookcorey, my_state, my_trid)

call mod_ctrl%sp%set_params(mod_ctrl%opts)

mod_ctrl%st%wtd = my_opts%initial_wtd

!sewt up the intial conditions
call mod_ctrl%initialize()
...
massb = sum( mod_ctrl%st%liq_vol*mod_ctrl%sp%dz(:), dim=1)

do j=1,mod_ctrl%npts
   write(21,*) mod_ctrl%st%liq_vol(j)
end do
...
do n=1,my_opts%ntime

   call mod_ctrl%update()

   dwat(:) = mod_ctrl%get_soln()

   call mod_ctrl%step_time()


end do      ! end time loop


masse = sum( mod_ctrl%st%liq_vol*mod_ctrl%sp%dz(:), dim=1)

masserror = masse - massb - mod_ctrl%opts%q_infil * mod_ctrl%opts%dtime*real(mod_ctrl%opts%ntime,kind=uk)
...
write(*,*) 'mass err per dt ',masserror/real(mod_ctrl%opts%ntime,kind=uk)

    
end program run_all_sims
